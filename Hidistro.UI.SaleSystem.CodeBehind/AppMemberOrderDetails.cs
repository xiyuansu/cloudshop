...
{
	[ParseChildren(true)]
	public class AppMemberOrderDetails : AppshopMemberTemplatedWebControl
	{
		...
		private HtmlInputHidden hidShowRedEnvelope;
		...
		private AppshopTemplatedRepeater rptPromotionGifts;
		...
		private void rptOrderProducts_ItemDataBound(object sender, RepeaterItemEventArgs e)
		{
			if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
			{
				...
				{
					htmlAnchor3.Attributes.Add("data-pid", num.ToString());
					htmlAnchor3.Attributes.Add("data-type", "2");
					AttributeCollection attributes = htmlAnchor3.Attributes;
					num2 = this.order.StoreId;
					attributes.Add("data-storeid", num2.ToString());
				}
				else
				{
					htmlAnchor.Attributes.Add("data-pid", num.ToString());
					htmlAnchor.Attributes.Add("data-type", "1");
				}
				literal.Text = lineItemInfo.ItemAdjustedPrice.F2ToString("f2");
				Literal literal4 = literal2;
				num2 = lineItemInfo.Quantity;
				...
				if (this.order.StoreId > 0)
				{
					htmlAnchor3.Attributes.Add("data-pid", num.ToString());
					htmlAnchor3.Attributes.Add("data-type", "2");
					AttributeCollection attributes2 = htmlAnchor3.Attributes;
					num2 = this.order.StoreId;
					attributes2.Add("data-storeid", num2.ToString());
				}
				else
				{
					htmlAnchor3.Attributes.Add("data-pid", num.ToString());
					htmlAnchor3.Attributes.Add("data-type", "1");
				}
				string text3 = htmlAnchor2.HRef = "ApplyReturn?OrderId=" + this.order.OrderId + "&SkuId=" + lineItemInfo.SkuId;
				literal3.Text = lineItemInfo.SKUContent;
				LineItemStatus lineItemStatus = (LineItemStatus)DataBinder.Eval(e.Item.DataItem, "Status");
				string text4 = (string)DataBinder.Eval(e.Item.DataItem, "StatusText");
				...
				{
					text4 = ((returnInfo.AfterSaleType != AfterSaleTypes.OnlyRefund) ? EnumDescription.GetEnumDescription((Enum)(object)returnInfo.HandleStatus, 1) : EnumDescription.GetEnumDescription((Enum)(object)returnInfo.HandleStatus, 3));
				}
				else if (replaceInfo != null)
				{
					text4 = ((replaceInfo.HandleStatus == ReplaceStatus.Replaced) ? "" : EnumDescription.GetEnumDescription((Enum)(object)replaceInfo.HandleStatus, 1));
				}
				...
				if (lineItemInfo.Status != 0 && this.order.OrderStatus != OrderStatus.Refunded && this.order.OrderStatus != OrderStatus.Closed)
				{
					...
					if (lineItemInfo.ReturnInfo != null && (lineItemInfo.Status == LineItemStatus.RefundApplied || lineItemInfo.Status == LineItemStatus.Refunded || lineItemInfo.Status == LineItemStatus.RefundRefused || lineItemInfo.Status == LineItemStatus.ReturnApplied || lineItemInfo.Status == LineItemStatus.Returned || lineItemInfo.Status == LineItemStatus.ReturnsRefused || lineItemInfo.Status == LineItemStatus.MerchantsAgreedForReturn || lineItemInfo.Status == LineItemStatus.GetGoodsForReturn || lineItemInfo.Status == LineItemStatus.DeliveryForReturn))
					{
						...
						AttributeCollection attributes3 = htmlAnchor4.Attributes;
						num2 = lineItemInfo.ReturnInfo.ReturnId;
						attributes3.Add("returnsId", num2.ToString());
					}
					else if (lineItemInfo.ReplaceInfo != null && lineItemInfo.ReplaceInfo.HandleStatus != ReplaceStatus.Replaced)
					{
						...
						htmlAnchor4.Visible = true;
						AttributeCollection attributes4 = htmlAnchor4.Attributes;
						num2 = lineItemInfo.ReplaceInfo.ReplaceId;
						attributes4.Add("replaceId", num2.ToString());
					}
