	...
	public class APPPointInfo : AppshopMemberTemplatedWebControl
	{
		...
		protected void btnClearCart_Click(object sender, EventArgs e)
		{
			...
			else if (int.Parse(this.litNeedPoints.Text) <= int.Parse(this.litHasPoints.Text))
			{
				...
				{
					this.Page.ClientScript.RegisterStartupScript(base.GetType(), "myscript", "<script>$(function(){alert_h(\"添加到购物车失败！\")});;</script>");
				}
			}
			else
			{
				this.Page.ClientScript.RegisterStartupScript(base.GetType(), "myscript", "<script>$(function(){alert_h(\"积分不足！\")});</script>");
			}
		}
	}
}
