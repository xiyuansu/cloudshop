...
namespace Hidistro.UI.SaleSystem.CodeBehind
{
	...
	public class WAPSubmmitOrder : WAPMemberTemplatedWebControl
	{
		...
		private WapTemplatedRepeater rptAddress;
		...
		private void CheckParamter()
		{
			...
			else
			{
				...
				else
				{
					...
					if (httpCookie != null && !string.IsNullOrEmpty(httpCookie.Value) && string.IsNullOrEmpty(this.productSku))
					{
						this.productSku = Globals.UrlDecode(httpCookie.Value);
						List<string> source = (from d in this.productSku.Split(',')
						where !string.IsNullOrWhiteSpace(d)
						select d).ToList();
						source = (from d in source
						select d.Split('|')[0]).ToList();
						this.productSku = string.Join(",", source);
					}
					if (this.CheckProductSkuHasProductPreInfo() && this.storeId <= 0)
					{
						this.hasError = true;
						this.ShowWapMessage("您所选商品中有预售商品，请重新选择！", "ShoppingCart");
						return;
					}
			...
			if (this.RecordId == 0)
			{
				...
				{
					this.cart = ShoppingCartProcessor.GetShoppingCart(this.from, this.productSku, this.buyAmount, this.combinaid, true, -1, this.Page.Request["fightGroupActivityId"].ToInt(0));
				}
				else
				{
					this.cart = ShoppingCartProcessor.GetShoppingCart(this.from, this.productSku, this.buyAmount, this.combinaid, true, this.storeId, this.Page.Request["fightGroupActivityId"].ToInt(0));
				}
			}
			else
			{
				...
				{
					this.ShowWapMessage("奖项不存在不存在", "Default.aspx");
					return;
				}
		...
		protected override void AttachChildControls()
		{
			...
			this.spandemo.Visible = this.siteSettings.IsDemoSite;
			...
			this.rptAddress = (WapTemplatedRepeater)this.FindControl("rptAddress");
			...
			this.hidFightGroupPickeupInStore.Value = this.siteSettings.FitGroupIsOpenPickeupInStore.ToNullString().ToLower();
			...
			if (this.cart != null && ((this.cart.LineItems != null && this.cart.LineItems.Count > 0) || (this.cart.LineGifts != null && this.cart.LineGifts.Count > 0)))
			{
				...
				int num = TradeHelper.WapPaymentTypeCount(base.ClientType, this.isFightGroup).ToInt(0);
				...
				if ((!this.siteSettings.EnableTax && !this.siteSettings.EnableE_Invoice && !this.siteSettings.EnableVATInvoice) || (this.cart != null && this.cart.LineItems.Count() == 0))
				{
				...
				decimal num2 = this.siteSettings.TaxRate;
				...
				if (this.siteSettings.EnableTax || this.siteSettings.EnableE_Invoice)
				{
					this.hidOrderRate.Value = ((this.siteSettings.TaxRate > decimal.Zero) ? (this.cart.GetTotal(this.storeId > 0) * this.siteSettings.TaxRate / 100m).F2ToString("f2") : "0.00");
				}
				else if (this.siteSettings.EnableVATInvoice)
				{
					this.hidOrderRate.Value = ((this.siteSettings.VATTaxRate > decimal.Zero) ? (this.cart.GetTotal(this.storeId > 0) * this.siteSettings.VATTaxRate / 100m).F2ToString("f2") : "0.00");
				}
				else
				{
				...
				if (flag2 && user != null && this.siteSettings.ShoppingDeduction > 0 && user.Points > 0)
				{
					int shoppingDeductionRatio = this.siteSettings.ShoppingDeductionRatio;
					decimal num5 = (decimal)shoppingDeductionRatio * this.cart.GetTotal(this.storeId > 0) * (decimal)this.siteSettings.ShoppingDeduction / 100m;
					int num6 = ((decimal)user.Points > num5) ? ((int)num5) : user.Points;
					decimal d = Math.Round((decimal)num6 / (decimal)this.siteSettings.ShoppingDeduction, 2);
					if (d > decimal.Zero && num6 > 0)
					{
						HtmlInputHidden htmlInputHidden6 = this.hidShoppingDeduction;
						int num7 = this.siteSettings.ShoppingDeduction;
						htmlInputHidden6.Value = num7.ToString();
						this.lblMaxPoints.Text = num6.ToString();
						this.lblMaxPointsToPrice.Text = (num6 / this.siteSettings.ShoppingDeduction).F2ToString("f2");
						this.hidCanPointUseWithCoupon.Value = (this.siteSettings.CanPointUseWithCoupon ? "true" : "false");
		...
		{
			List<ViewCombinationBuySkuInfo> combinaSkusInfoByCombinaId = CombinationBuyHelper.GetCombinaSkusInfoByCombinaId(this.combinaid);
			...
			if (startDate > date || endDate < date)
			{
				if (!this.hasError)
				{
					this.ShowWapMessage("未到活动时间或者活动已结束", "");
				}
				return false;
			}
			...
			foreach (string item in array2)
			{
				...
				if (viewCombinationBuySkuInfo == null)
				{
					if (!this.hasError)
					{
						this.ShowWapMessage("订单中有商品不在组合购产品中,请联系管理员！", "");
					}
					return false;
				}
			...
			if (this.CheckProductSkuHasProductPreInfo())
			{
				if (!this.hasError)
				{
					this.ShowWapMessage("订单中有商品有预售商品，请联系管理员！", "");
				}
				return false;
			}
			return true;
