...
namespace Hidistro.UI.SaleSystem.CodeBehind
{
	public class ForgotTradePassword : MemberTemplatedWebControl
	{
		...
		private void btnSendMobile_Click(object sender, EventArgs e)
		{
			...
			if (user != null)
			{
				string code = HiContext.Current.CreatePhoneCode(6, user.CellPhone, VerifyCodeType.Digital);
				string sendretmsg ="";
                string TemplateParam = string.Format("{{\"code\":\"{0}\"}}", code);//短信模板变量替换JSON串
                SendStatus sendStatus = Messenger.SendSMS(user.CellPhone, TemplateCode, TemplateParam, siteSettings, out sendretmsg);
