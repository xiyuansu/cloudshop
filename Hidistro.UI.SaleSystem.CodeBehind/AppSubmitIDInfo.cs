...
{
	public class AppSubmitIDInfo : AppshopMemberTemplatedWebControl
	{
		...
		private void Binding()
		{
			...
			int.TryParse(this.Page.Request.QueryString["ShippingId"], out AppSubmitIDInfo.shippingId);
			if (AppSubmitIDInfo.shippingId > 0)
			{
				ShippingAddressInfo shippingAddress = MemberProcessor.GetShippingAddress(AppSubmitIDInfo.shippingId);
				if (shippingAddress != null)
				{
					this.hidshippingId.Value = AppSubmitIDInfo.shippingId.ToString();
