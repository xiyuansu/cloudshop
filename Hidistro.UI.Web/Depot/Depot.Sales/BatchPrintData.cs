...
{
	public class BatchPrintData : StoreAdminPage
	{
		...
		protected Literal ltlShipTo;

		protected Literal ltlRegion;

		protected Literal ltlAddress;

		protected Literal ltlCellPhone;
		...
		private string GetDefinedDataItem(string name)
		{
			...
			foreach (XmlNode item in xmlNodeList)
			{
				...
				{
					return this.ReplaceString(item.ChildNodes[1].InnerText.Trim());
				}
			}
			return string.Empty;
		}
		...
		private IList<OrderInfo> GetPrintDataList(string orderIds)
		{
			...
			foreach (string orderId in array)
			{
				...
				if (orderInfo != null && orderInfo.ShippingModeId != -2 && orderInfo.StoreId == this.UserStoreId && orderInfo.ItemStatus == OrderItemStatus.Nomarl && (orderInfo.OrderStatus == OrderStatus.BuyerAlreadyPaid || (orderInfo.Gateway == "hishop.plugins.payment.podrequest" && orderInfo.OrderStatus == OrderStatus.WaitBuyerPay) || orderInfo.OrderStatus == OrderStatus.SellerAlreadySent))
				{
					list.Add(orderInfo);
				}
			}
		...
		private void LoadShipper()
		{
			...
			if (storeById != null)
			{
				this.ltlAddress.Text = storeById.Address;
				this.ltlCellPhone.Text = storeById.Tel;
				this.ltlShipTo.Text = storeById.ContactMan;
				this.ltlRegion.Text = RegionHelper.GetFullRegion(storeById.RegionId, " ", true, 0);
