	...
	public class RegisterHiPOSPay : AdminPage
	{
			...
			select c).ForEach(delegate(PaymentModeInfo wxPayment)
			{
				...
				else
				{
					...
					if (string.IsNullOrEmpty(this.lblShowCertPath.Text) && !string.IsNullOrEmpty(text3))
					{
						...
						this.lblShowCertPath.Text = text4 + " 删除重传";
		...
		protected void btnAdd_Click(object sender, EventArgs e)
		{
			...
			if (masterSettings.EnableHiPOSWX)
			{
				...
				{
					string text9 = masterSettings.HiPOSWXCertPath = this.lblShowCertPath.Text.Replace("删除重传", string.Empty).Trim();
				}
			}
			else
			{
			...
			{
				this.ShowMsg("提交商户支付方式成功", true, "RegisterHiPOSFinished.aspx");
			}
			else
			{
