...
{
	public class BatchPrintData : AdminCallBackPage
	{
		...
		protected string SizeStreet = string.Empty;
		...
		protected string ShipSizeStreet = string.Empty;

		protected string ShipStreet = string.Empty;
		...
		private void PrintPage(string pagewidth, string pageheght)
		{
			...
			if (!string.IsNullOrEmpty(this.SizeStreet.Trim()))
			{
				stringBuilder.Append(" var Streets=[" + this.Street.Substring(0, this.Street.Length - 1) + "];var SizeStreet=[" + this.SizeStreet.Substring(0, this.SizeStreet.Length - 1) + "];");
			}
			...
			if (!string.IsNullOrEmpty(this.ShipSizeStreet.Trim()))
			{
				stringBuilder.Append(" var ShipStreet=[" + this.ShipStreet.Substring(0, this.ShipStreet.Length - 1) + "];var ShipSizeStreet=[" + this.ShipSizeStreet.Substring(0, this.ShipSizeStreet.Length - 1) + "];");
			}
			...
			if (!string.IsNullOrEmpty(this.SizeStreet.Trim()))
			{
				stringBuilder.Append(" LODOP.ADD_PRINT_TEXT(SizeStreet[i].split(',')[0],SizeStreet[i].split(',')[1],SizeStreet[i].split(',')[2],SizeStreet[i].split(',')[3],Street[0]);");
			}
			if (!string.IsNullOrEmpty(this.SizeShipToDate.Trim()))
			{
			...
			if (!string.IsNullOrEmpty(this.ShipSizeStreet.Trim()))
			{
				stringBuilder.Append(" LODOP.ADD_PRINT_TEXT(ShipSizeStreet[i].split(',')[0],ShipSizeStreet[i].split(',')[1],ShipSizeStreet[i].split(',')[2],ShipSizeStreet[i].split(',')[3],ShipStreet[i]);");
			}
			if (!string.IsNullOrEmpty(this.SizeDeparture.Trim()))
			{
			...
			else
			{
				...
				if (File.Exists(text))
				{
					...
					foreach (OrderInfo item in printDataList)
					{
						...
						string[] array = item.ShippingRegion.ToNullString().Split('，');
						if (item.ShippingRegion.ToNullString().IndexOf("，") == -1)
						{
							array = item.ShippingRegion.ToNullString().Split(',');
						}
						foreach (XmlNode item2 in xmlNode.SelectNodes("item"))
						{
							...
							if (innerText3.Split('_')[0] == "收货人-地区4级")
							{
								text2 = string.Empty;
								if (array.Length > 3)
								{
								...
								this.ShipStreet = this.ShipStreet + "'" + text2 + "',";
								if (!string.IsNullOrEmpty(text2.ToNullString().Trim()))
								{
									this.ShipSizeStreet = this.ShipSizeStreet + "'" + str + "',";
								}
								else
								{
									this.ShipSizeStreet += "'0,0,0,0',";
								}
							}
							...
							if (innerText3.Split('_')[0] == "发货人-地区4级")
							{
								text7 += string.Empty;
								if (array2.Length > 3)
								{
								...
								this.Street = this.Street + "'" + text7 + "',";
								if (!string.IsNullOrEmpty(text7.ToNullString().Trim()))
								{
									this.SizeStreet = this.SizeStreet + "'" + str + "',";
								}
								else
								{
									this.SizeStreet += "'0,0,0,0',";
								}
							}
		...
		{
			if (HiContext.Current.Manager.RoleId == -1 || HiContext.Current.Manager.RoleId == -3)
			{
		...
		private IList<OrderInfo> GetPrintDataList(string orderIds)
		{
			...
			foreach (string orderId in array)
			{
				...
				if (orderInfo != null && orderInfo.StoreId == this.UserStoreId && orderInfo.ItemStatus == OrderItemStatus.Nomarl && (orderInfo.OrderStatus == OrderStatus.BuyerAlreadyPaid || (orderInfo.Gateway == "hishop.plugins.payment.podrequest" && orderInfo.OrderStatus == OrderStatus.WaitBuyerPay) || orderInfo.OrderStatus == OrderStatus.SellerAlreadySent))
				{
					list.Add(orderInfo);
				}
			}
