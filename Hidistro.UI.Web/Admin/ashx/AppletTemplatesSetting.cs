...
{
	public class AppletTemplatesSetting : AdminBaseHandler
	{
		public override void OnLoad(HttpContext context)
		{
			...
			switch (base.action)
			{
			case "quicksetweixintemplates":
				this.QuickSetWeixinTemplates(context);
				break;
			case "saveweixintemplates":
				this.SaveWeiXinTemplates(context);
				break;
			case "saveapplettemplates":
				this.SaveAppletTemplates(context);
				break;
			case "saveo2oapplettemplates":
				this.SaveO2OAppletTemplates(context);
				break;
			default:
				throw new HidistroAshxException("错误的参数");
			}
		...
		public void SaveO2OAppletTemplates(HttpContext context)
		{
			...
			try
			{
				...
				if (savePostData.TemplatePostData != null && savePostData.TemplatePostData.Count > 0)
				{
					...
					foreach (WXTemplateInfo templatePostDatum in savePostData.TemplatePostData)
					{
						...
						messageTemplate.WxO2OAppletTemplateId = templatePostDatum.TemplateId;
						list.Add(messageTemplate);
					}
					flag = WeiXinTemplateProcessor.SaveO2OAppletTempalteIdOfMsgType(list);
				}
				else
				{
			...
			catch (Exception ex)
			{
				...
				Globals.WriteExceptionLog(ex, dictionary, "SaveAppletTemplates");
			}
			string s2 = JsonConvert.SerializeObject(new
			{
				Status = (flag ? "SUCCESS" : "FAIL"),
				Message = "保存配置" + (flag ? "成功" : "失败")
			});
			context.Response.Write(s2);
			context.Response.End();
		}

		public void SaveAppletTemplates(HttpContext context)
		{
			SiteSettings masterSettings = SettingsManager.GetMasterSettings();
			if (masterSettings.IsDemoSite)
			{
			...
			try
			{
				...
				if (savePostData.TemplatePostData != null && savePostData.TemplatePostData.Count > 0)
				{
					...
					foreach (WXTemplateInfo templatePostDatum in savePostData.TemplatePostData)
					{
						MessageTemplate messageTemplate = new MessageTemplate();
						messageTemplate.MessageType = templatePostDatum.MessageType;
						messageTemplate.WxAppletTemplateId = templatePostDatum.TemplateId;
						list.Add(messageTemplate);
					}
					flag = WeiXinTemplateProcessor.SaveAppletTempalteIdOfMsgType(list);
				}
				else
				{
					Globals.AppendLog(parameter, "提交的数据为空", "", "SaveWeiXinTemplates");
