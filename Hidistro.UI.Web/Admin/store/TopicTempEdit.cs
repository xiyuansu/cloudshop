...
namespace Hidistro.UI.Web.Admin.store
{
	...
	public class TopicTempEdit : Page
	{
		...
		protected void Page_Load(object sender, EventArgs e)
		{
			...
			if (!string.IsNullOrEmpty(this.Page.Request["TopicId"]))
			{
				...
				if (this.Page.Request["TopicId"].ToInt(0) > 0)
				{
					TopicInfo topicInfo = VShopHelper.Gettopic(this.Page.Request["TopicId"].ToInt(0));
					if (topicInfo != null)
					{
						this.hidOldImages.Value = topicInfo.SharePic;
					}
				}
			}
			if (!base.IsPostBack)
		...
		{
			string path = base.Server.MapPath("/Templates/vshop/data/default.json");
