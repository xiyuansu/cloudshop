...
{
	[PrivilegeCheck(Privilege.VShopTempEdit)]
	public class ShopTempEditNew : Page
	{
		public string scriptSrc = "/Templates/common/home/";

		protected HtmlLink cssLink;

		protected HtmlForm thisForm;

		protected HtmlInputHidden themeName;
		...
		protected void Page_Load(object sender, EventArgs e)
		{
			...
			if (!base.IsPostBack)
			{
				this.scriptSrc = this.scriptSrc + this.Page.Request.QueryString["theme"].Trim() + "/script/head.js";
				this.dropCategories.DataBind();
				this.hidOpenMultStore.Value = (SettingsManager.GetMasterSettings().OpenMultStore ? "1" : "0");
				this.themeName.Value = this.Page.Request.QueryString["theme"].Trim();
			}
		}
		...
		{
			string path = base.Server.MapPath("/Templates/vshop/" + this.Page.Request.QueryString["theme"].Trim() + "/data/default.json");
