...
{
	public class ReceivedMessages : AdminBaseHandler
	{
		...
		private DataGridViewModel<Dictionary<string, object>> GetDataList(MessageBoxQuery query)
		{
			...
			if (query != null)
			{
				DbQueryResult managerReceivedMessages = NoticeHelper.GetManagerReceivedMessages(query);
				dataGridViewModel.rows = DataHelper.DataTableToDictionary(managerReceivedMessages.Data);
				dataGridViewModel.total = managerReceivedMessages.TotalRecords;
