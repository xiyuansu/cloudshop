	...
	public class SetBrandCategoryTemplate : AdminPage
	{
		...
		private void btnUpload_Click(object sender, EventArgs e)
		{
			if (this.fileThame.HasFile)
			{
				...
				{
					string virtualPath = HiContext.Current.GetSkinPath() + "/brandcategorythemes/" + SetBrandCategoryTemplate.GetFilename(Path.GetFileName(this.fileThame.PostedFile.FileName), Path.GetExtension(this.fileThame.PostedFile.FileName));
		...
		{
			foreach (RepeaterItem item in this.rptBrandes.Items)
			{
		...
		private void BindData()
		{
			...
			this.rptBrandes.DataSource = brandCategories;
			this.rptBrandes.DataBind();
		}
		...
		private void SaveAll()
		{
			...
			{
				string selectedValue = (item.FindControl("dropTheme") as DropDownList).SelectedValue;
				int brandid = (item.FindControl("hidBrandId") as TextBox).Text.ToInt(0);
				CatalogHelper.SetBrandCategoryThemes(brandid, selectedValue);
			}
		}
		...
		protected void rptBrandes_ItemCommand(object source, RepeaterCommandEventArgs e)
		{
			if (e.CommandName == "Save")
			{
				TextBox textBox = e.Item.FindControl("hidBrandId") as TextBox;
				int brandid = textBox.Text.ToInt(0);
				DropDownList dropDownList = (DropDownList)e.Item.FindControl("dropTheme");
				string selectedValue = dropDownList.SelectedValue;
				if (CatalogHelper.SetBrandCategoryThemes(brandid, selectedValue))
				{
					this.BindData();
					this.ShowMsg("保存品牌模板成功", true);
				}
			}
		}

		protected void rptBrandes_ItemDataBound(object sender, RepeaterItemEventArgs e)
		{
			if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
			{
				DataRowView dataRowView = (DataRowView)e.Item.DataItem;
				DropDownList box = (DropDownList)e.Item.FindControl("dropTheme");
				this.BindDorpDown(box, new ListItem("无", ""), dataRowView["Theme"].ToString());
			}
		}
	}
}
