...
namespace Hidistro.UI.Web.Admin.Supplier.Order
{
	...
	public class ReturnApplyDetail : AdminPage
	{
		...
		public void bindReturnInfo()
		{
			...
			if (returnInfo.HandleStatus == ReturnStatus.Applied && (orderInfo.StoreId == this.UserStoreId || orderInfo.StoreId < 0))
			{
				this.btnAcceptReturn.Visible = true;
				this.btnRefuseReturn.Visible = true;
			}
			if (orderInfo.IsStoreCollect && (orderInfo.StoreId == this.UserStoreId || orderInfo.StoreId < 0))
			{
				goto IL_03aa;
			}
			...
			goto IL_03b6;
			IL_03aa:
			num2 = ((returnInfo.HandleStatus == ReturnStatus.GetGoods) ? 1 : 0);
			goto IL_03b6;
			IL_03b6:
			if (num2 != 0)
			{
		...
		private void btnRefuseReturn_Click(object sender, EventArgs e)
		{
			...
			else
			{
				...
				else
				{
					...
					if (orderInfo == null || (orderInfo.StoreId != this.UserStoreId && orderInfo.StoreId >= 0))
					{
