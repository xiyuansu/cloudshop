		...
		protected override void OnLoad(EventArgs e)
		{
			...
			if (this.order.OrderStatus == OrderStatus.SellerAlreadySent)
			{
				if (this.setting.OpenMultStore)
				{
					...
					{
						this.edit = "&nbsp;<input type=\"button\" class=\"btn btn-default\" id=\"btnedit\" value=\"修改发货单号\" onclick=\"ShowPurchaseOrder();\"/>";
					}
					else if (this.IsStoreAdminView)
					{
						this.edit = "&nbsp;<input type=\"button\" class=\"btn btn-default\" id=\"btnedit\" value=\"修改发货单号\" onclick=\"ShowPurchaseOrder();\"/>";
					}
				}
