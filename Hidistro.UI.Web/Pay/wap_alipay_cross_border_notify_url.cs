...
{
	public class wap_alipay_cross_border_notify_url : Page
	{
		...
		private const string LOG_FILE_NAME = "alipay_forex_wap";
		...
		protected void Page_Load(object sender, EventArgs e)
		{
			...
			else
			{
				...
				else
				{
					...
					if (paymentMode == null)
					{
						Globals.WriteLog(nameValueCollection, "支付方式获取失败hishop.plugins.payment.alipaycrossbordermobilepayment.alipaycrossbordermobilepaymentrequest", "", "", "alipay");
						base.Response.Write("success");
					}
					else
					{
			...
			else
			{
				...
				else
				{
					Globals.WriteLog(new NameValueCollection
					{
						this.Page.Request.Form,
						this.Page.Request.QueryString
					}, "订单不是待支付状态，或者更新订单状态失败", "", "", "alipay_forex_wap");
					base.Response.Write("success");
				}
			}
		}
