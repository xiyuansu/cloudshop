...
namespace Hishop.Components.Validation
{
	internal class ValidatorBuilderBase
	{
		...
		{
			Validator validator = this.CreateValidatorForValidatedElement(validatedType, this.GetCompositeValidatorBuilderForType);
			if (validator != null)
			{
		...
		private void CollectValidatorsForProperties(IEnumerable<IValidatedElement> validatedElements, List<Validator> validators, Type ownerType)
		{
			...
			{
				Validator validator = this.CreateValidatorForValidatedElement(validatedElement, this.GetCompositeValidatorBuilderForProperty);
				if (validator != null)
				{
					validators.Add(validator);
				}
		...
		private void CollectValidatorsForFields(IEnumerable<IValidatedElement> validatedElements, List<Validator> validators, Type ownerType)
		{
			foreach (IValidatedElement validatedElement in validatedElements)
			{
				Validator validator = this.CreateValidatorForValidatedElement(validatedElement, this.GetCompositeValidatorBuilderForField);
				if (validator != null)
				{
					validators.Add(validator);
				}
		...
		private void CollectValidatorsForMethods(IEnumerable<IValidatedElement> validatedElements, List<Validator> validators, Type ownerType)
		{
			foreach (IValidatedElement validatedElement in validatedElements)
			{
				Validator validator = this.CreateValidatorForValidatedElement(validatedElement, this.GetCompositeValidatorBuilderForMethod);
				if (validator != null)
				{
					validators.Add(validator);
				}
