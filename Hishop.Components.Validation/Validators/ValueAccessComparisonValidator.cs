...
namespace Hishop.Components.Validation.Validators
{
	public class ValueAccessComparisonValidator : ValueValidator
	{
		...
		protected internal override void DoValidate(object objectToValidate, object currentTarget, string key, ValidationResults validationResults)
		{
			...
			{
				base.LogValidationResult(validationResults, string.Format(CultureInfo.CurrentCulture, Resources.ValueAccessComparisonValidatorFailureToRetrieveComparand, this.valueAccess.Key, text), currentTarget, key);
			}
			else
			{
				bool flag = false;
				...
				{
					flag = ((objectToValidate?.Equals(obj) ?? (obj == null)) ^ this.comparisonOperator == ComparisonOperator.NotEqual ^ base.Negated);
				}
				else
				{
				...
				if (!flag)
				{
					base.LogValidationResult(validationResults, string.Format(CultureInfo.CurrentCulture, base.MessageTemplate, objectToValidate, key, base.Tag, obj, this.valueAccess.Key, this.comparisonOperator), currentTarget, key);
				}
			}
		}
	}
