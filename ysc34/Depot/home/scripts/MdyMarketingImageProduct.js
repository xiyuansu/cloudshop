...
function doSubmit() {
    ...
    $.post('/Depot/home/ashx/MarketingImageManage.ashx', { flag: "Mdy", ImageId: ImageId, ProductIds: ProductIds }, function (json) {
        var obj = eval('(' + json + ')');
        if (obj.Result != undefined) {
            if (obj.Result.Success.Status == true) {
                alert("配置成功");
                window.location.href = "/Depot/home/MarketingImageManage.aspx";
            }
            else {
                alert(obj);
                $("#divLoading").hide();
