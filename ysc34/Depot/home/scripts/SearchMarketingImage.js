...
function ShowSuccessAndCloseReload(id) {
    ShowMsg("营销图片" + (id > 0 ? "编辑" : "添加") + "成功", true);
    CloseDialogAndReloadData();
}

function bat_deletes() {
    var ids = GetSelectId();
    if (ids.length == 0) {
        alert("参数错误");
        return;
...
function Add() {
    DialogFrame('depot/Setting/MarketingImageInfo.aspx?callback=ShowSuccessAndCloseReload', '添加营销图片', 450, 350, null);
}
function Edit(id) {
    DialogFrame('depot/Setting/MarketingImageInfo.aspx?ImageId=' + id + "&callback=ShowSuccessAndCloseReload", '编辑营销图片', 450, 350, null);
}

function CheckImage(id,name)
{
    var origin = artDialog.open.origin;
    $(origin.document.getElementById("ctl00_contentHolder_spGLYXTP")).html(name);
    $(origin.document.getElementById("ctl00_contentHolder_spGLYXTPID")).html(id);
    art.dialog.close();
}