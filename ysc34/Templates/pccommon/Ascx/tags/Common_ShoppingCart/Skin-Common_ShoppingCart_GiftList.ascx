                ...
                <div class="floor" id='ck_<%#Eval("GiftId") %>'>
                    <div class="con">
                        <div class="select" id="divCheck" runat="server">
                            <input type="checkbox" checked="checked" class="icheck" id="ck_<%# Eval("GiftId") %>" name="ck_gifId" value="<%# Eval("GiftId") %>" />
                        </div>
                        <div class="name">
...
<script type="text/javascript">
    ...
    function removeCartGiftItem(giftId) {
        if (confirm('确定删除？')) {
            $.ajax({
                ...
                dataType: "json",
                timeout: 10000,
                data: { action: 'deleteGift', giftId: giftId, client: "pc" },
                async: false,
    ...
    function ChangeGiftNum(txtBuyNum) {
        ...
        $.ajax({
            ...
            dataType: "json",
            timeout: 10000,
            data: { action: 'updateGiftBuyNum', giftId: giftId, BuyNum: quantity, client: "pc" },
            async: false,
            success: function (data) {
                if (data.status == 'true') {
