        ...
        <div>
            <span class="s_span2">市场价格：<Hi:FormatedMoneyLabel ID="FormatedMoneyLabel1" Money='<%# Eval("MarketPrice").ToString()==""?0:(decimal)Eval("MarketPrice") %>'
                        runat="server" /></span> <span class="s_span2">你的价格：<Hi:FormatedMoneyLabel ID="FormatedMoneyLabel2"
                            Money='<%# Eval("SalePrice") %>' runat="server" /></span>
        </div>
        <div class="s_span1" <%# Eval("MarketPrice").ToDecimal()>0?"":"style=\"display:none\"" %>>
            节省：  <Hi:FormatedMoneyLabel ID="FormatedMoneyLabel3" Money='<%# Math.Abs((Eval("MarketPrice").ToString()==""?0:(decimal)Eval("MarketPrice"))-(decimal)Eval("SalePrice") )%>'
