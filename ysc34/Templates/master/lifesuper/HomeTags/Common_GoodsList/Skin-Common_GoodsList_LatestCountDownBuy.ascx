...
<ItemTemplate>
 ...
 <li>
      <div class="time o-hidden"><span id='<%# "htmlspan"+Eval("CountDownId") %>'></span>
 <Hi:LeaveListTime runat="server" ID="LeaveListTime" /></div>
            
      <div class="pic mt10"> <Hi:ProductDetailsLink ID="ProductDetailsLink1" runat="server" ProductName='<%# Eval("ProductName") %>' ProductId='<%# Eval("ProductId") %>' ImageLink="true" href='<%#"/CountDownProductsDetails.aspx?countDownId="+ Eval("CountDownId") %>'>
        ...
        ProductId='<%# Eval("ProductId") %>' ImageLink="false"/></div>
            <div class="price"><b><em>￥</em><Hi:FormatedMoneyLabel runat="server" ID="lblPrice" Money='<%# Eval("CountDownPrice") %>'/></b></div>
