...
<asp:Repeater ID="repcountdown" runat="server">
    ...
    <li>
        ...
        <div class="time">
            <span id='<%# "htmlspan"+Eval("CountDownId") %>'></span><Hi:LeaveListTime runat="server" ID="LeaveListTime" />
        </div>
        ...
        <div class="price">
            <em>￥</em><Hi:FormatedMoneyLabel runat="server" ID="lblPrice" Money='<%# Eval("CountDownPrice") %>'/>
            <div style="display:none;"><i>¥<Hi:FormatedMoneyLabel runat="server" ID="lblMarketPrice" Money='<%#Eval("MarketPrice").ToInt()>0?Eval("MarketPrice"):Eval("SalePrice") %>' /></i></div>
        </div>
