...
<asp:Repeater runat="server" ID="recordsone">
     <ItemTemplate>
		...
		<div id='<%# "twoCategory_" + Eval("CategoryId")%>' class="h2_cat"  >
		    <h3> <em><img class="size" src="<%#Eval("Icon") %>" /> </em> <a href='<%# RouteConfig.SubCategory(Convert.ToInt32(Eval("CategoryId")), Eval("RewriteName")) %>'><%# Eval("Name")%></a> </h3> <em style="display:none;"><asp:Repeater runat="server" ID="repMainTow">
                        <ItemTemplate>
