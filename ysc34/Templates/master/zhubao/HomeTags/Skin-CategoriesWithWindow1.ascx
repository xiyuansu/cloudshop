...
<asp:Repeater runat="server" ID="recordsone">
     <ItemTemplate>
		...
		<div id='<%# "twoCategory_" + Eval("CategoryId")%>' class="h2_cat"  >
		    <h3> <em><asp:Literal ID="Literal1" runat="server" Text='<%#Eval("Notes5")%>'  /></em> <a href='<%# RouteConfig.SubCategory(Convert.ToInt32(Eval("CategoryId")), Eval("RewriteName")) %>'><%# Eval("Name")%></a> </h3>
            <em style="display:none;"><asp:Repeater runat="server" ID="repMainTow">
                        <ItemTemplate>
