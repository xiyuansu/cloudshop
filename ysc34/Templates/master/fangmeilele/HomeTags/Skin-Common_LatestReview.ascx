...
<%@ Register TagPrefix="Hi" Namespace="Hidistro.UI.SaleSystem.Tags" Assembly="Hidistro.UI.SaleSystem.Tags" %>
 <li>
    ...
    <span class="colorA" style=" color:#ff6600;"><i style="width:30px;height:30px;display: block;float:left;padding-right:5px;"></i><%# string.IsNullOrEmpty(Eval("UserName").ToString())?"":(Eval("UserName").ToString().Substring(0,1)+"***"+Eval("UserName").ToString().Substring(Eval("UserName").ToString().Length-1,1)) %></span> </td><td><span class="colorC" style=" display:none;">评论时间：<%# Eval("ReviewDate")%></span>
