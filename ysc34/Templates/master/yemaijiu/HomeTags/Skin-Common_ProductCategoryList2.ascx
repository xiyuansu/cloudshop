...
<li>
    ...
    <div><b><a href='<%# RouteConfig.SubCategory(Convert.ToInt32(Eval("CategoryId")), Eval("RewriteName")) %>'>全部</a></b> 
     <asp:Repeater ID="rptSubCategries" runat="server">
        <ItemTemplate>
		   <a href='<%# RouteConfig.SubCategory(Convert.ToInt32(Eval("CategoryId")), Eval("RewriteName")) %>'><asp:Literal ID="litName" runat="server" Text='<%# Eval("Name")%>'></asp:Literal></a><span>|</span>
        </ItemTemplate>
    </asp:Repeater>
   </div>
</li>