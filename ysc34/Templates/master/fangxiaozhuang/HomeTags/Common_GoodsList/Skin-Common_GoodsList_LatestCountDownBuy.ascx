...
<asp:Repeater ID="repcountdown" runat="server">
    <ItemTemplate>
        ...
        <Hi:ListImage ID="HiImage1" runat="server" DataField="ThumbnailUrl180" IsDelayedLoading="true" /></Hi:ProductDetailsLink></span>
            <div class="pro-name">
            ...
            <div class="pro-price"><span><i class="rmb">¥</i><Hi:FormatedMoneyLabel runat="server" ID="lblPrice" Money='<%# Eval("CountDownPrice") %>' /></span><i>¥<Hi:FormatedMoneyLabel runat="server" ID="lblMarketPrice" Money='<%#Eval("MarketPrice").ToInt()>0?Eval("MarketPrice"):Eval("SalePrice") %>' /></i></div>
            <div class="buybtn"> <Hi:ProductDetailsLink ID="ProductDetailsLink3" runat="server"  ProductName='<%# Eval("ProductName") %>' ProductId='<%# Eval("ProductId") %>' ImageLink="true" CssClass="btnbuy" href='<%#"/CountDownProductsDetails.aspx?countDownId="+ Eval("CountDownId") %>'>立即抢购</Hi:ProductDetailsLink></div>
