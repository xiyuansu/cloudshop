    ...
    authPhonecode: function (option) {
        ...
        $.getJSON("Handler/MemberHandler.ashx?action=CheckAuthPhonecode&code=" + escape(option.value) + "&phone=" + phone + "&r=" + Math.random(), function (date) {
            if (date.success == "true") {
                ...
                validateSettings.error.run(option, date.msg);
...
$("#register_btnRegister").click(function () {
    ...
    if (flag) {
        if ($("#hidIsOpenGeetest").val() == "0" && $("#txtNumber").val().trim().length == 0) {
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                if (result.success == "true") {
                    ...
                    $("#register_btnRegister").removeAttr("disabled").addClass("xieyi_btn").removeClass("xieyi_submit_btn");
                    refreshCode();
