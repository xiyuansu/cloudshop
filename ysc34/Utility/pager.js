...
function vshopPager(containerId, maxSize, defaultPageNumber, defaultPageSize) {
    ...
    if (page == Math.ceil(maxSize * 1.0 / size) || maxSize == 0)
    ...
    var html = ' <ul class="pager">\
            <li id="previous" class="previous ' + preDisabled + '"><a href="' + preUrl + '">上一页</a></li>\
            <li id="next" class="next ' + nextDisabled + '"><a href="' + nextUrl + '">下一页</a></li>\
        </ul>';

    $('#' + containerId).html(html);

    $(function () {
        if ($("#next").hasClass("disabled") && $("#previous").hasClass("disabled")) {
            $("#pager").hide();
        }
    })
}
