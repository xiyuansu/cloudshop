...
(function ($) {
    function uploadfile(target, opts) {
        ...
        try {
            ...
            EXIF.getData(file, function () {
                ...
                postform(target, opts, Orientation);
            });
        } catch (error) {
            postform(target, opts, 0);
        }
        //准备表当
        ...
        form.ajaxSubmit({
            success: function (data) {               
                ...
                if ($(data).html().toString().length > $(data).html().toString().replace("error:", "").length) {
                    if ($("#span_error").length > 0) {
                        $("#span_error").text($(data).html().split(':')[1]);
                    }
                    else {
                        alert_h($(data).html().split(':')[1]);
                    }
                } else {
                    //文件上传成功，返回图片的路径。将路经赋给隐藏域
                    var filesData = $(data).html();
                    ...
                    for (var i = 0, j = 0; i < opts.imagesCount; i++) {
                        ...
                        if (started && files[j]) {
                            ...
                            $(uploadBtns[i]).hide();
                            $(imageBoxes[i]).show();
                            $(imageBoxes[i]).find(".remove-img").show();
                            $(imageBoxes[i]).find(".remove-img").click(function () {
    ...
    function postform(target, opts, orientation) {
        var imgupfile = $(target).find("input:file").eq(0); //上传控件       
        //准备表当
        var myform = document.createElement("form");
        var url = opts.url;
        ...
        hidMaxSize.value = (opts.fileMaxSize == undefined || isNaN(parseFloat(opts.fileMaxSize))) ? defaultMaxSize : parseFloat(opts.fileMaxSize);
        $(hidMaxSize).appendTo(form);
        var hidOrientation = document.createElement("input");
        hidOrientation.type = "hidden";
        hidOrientation.name = "hidOrientation";
        hidOrientation.value = orientation;
        $(hidOrientation).appendTo(form);
        if ($("#span_error").length > 0) {
            $("#span_error").text("");
        }
    ...
    function removepic(el) {
        ...
        $("img.img-upload", removeImg.parent()).attr("src", "").hide();
        $("div.glyphicon-plus", removeImg.parent()).show();
        $("input.hiddenImgSrc", removeImg.parent()).val("");
        $("input.file", removeImg.parent()).val("");
        //$(this).hide();
        changeTarget("", opts.target, opts.targetType);
    }
    ...
    var renderView = {
        render: function (opts, container) {
            ...
            for (var i = 0; i < opts.imagesCount; i++) {
                ...
                var imgHideUrl = opts.displayImgSrc[i];
