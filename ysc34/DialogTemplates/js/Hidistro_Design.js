...
var Hidistro_designer = {
    ...
    Design_Page_Init: function () {
        $.ajax({
            ...
            data: {
                ...
                Params: Hidistro_designer.CurrentParams
            },
            dataType: "json",
            ...
            complete: function () {
                ...
                $(".cssEdite").each(function (index, item) {
                    ...
                    var isparent = $(item).attr("isparent");
                    Hidistro_designer.IsParent = false;
                    if (isparent == "true") {
                        Hidistro_designer.IsParent = true;
                    }
    ...
    MoveUp: function () {
        ...
        var moveid = "";
        var movetype = "";
        moveid = $("#" + Hidistro_designer.CurrentContentID).prev().attr("id");
        movetype = $("#" + Hidistro_designer.CurrentContentID).prev().attr("type");
        if (moveid != "" && moveid != undefined) {
            Hidistro_designer.MoveElementId = moveid;
            Hidistro_designer.MoveType = movetype;
            $("#" + moveid).before("<div id='moveDiv'></div>");
            $("#" + Hidistro_designer.CurrentContentID).hide();
            $("#moveDiv").replaceWith($("#" + Hidistro_designer.CurrentContentID));
            $("#" + Hidistro_designer.CurrentContentID).show();
        }
        var a = $("#" + Hidistro_designer.CurrentContentID).width() - 175; //bar与div宽度差值
        $(groupdiv).css({ 'top': $("#" + Hidistro_designer.CurrentContentID).offset().top, 'left': $("#" + Hidistro_designer.CurrentContentID).offset().left }); //设置编辑层位置，在被编辑层上
        $(assitsdiv).css({ 'top': $("#" + Hidistro_designer.CurrentContentID).offset().top, 'left': $("#" + Hidistro_designer.CurrentContentID).offset().left }); //设置编辑层位置，在被编辑层上
        $(groudedite).css({ 'top': $("#" + Hidistro_designer.CurrentContentID).offset().top, 'left': $("#" + Hidistro_designer.CurrentContentID).offset().left + a }); //设置编辑层位置，在被编辑层上
        $(ctrnamediv).css({ 'top': $("#" + Hidistro_designer.CurrentContentID).offset().top + ($("#" + Hidistro_designer.CurrentContentID).outerHeight() / 2 - 12), 'left': $("#" + Hidistro_designer.CurrentContentID).offset().left + ($("#" + Hidistro_designer.CurrentContentID).outerWidth() / 2) - 80 });
        Hidistro_designer.MoveButtonState();
        Hidistro_designer.EditeMove(1);

    }, // 向上移动
    MoveDown: function () {
        if ($(a_down).hasClass("downdisable")) {
            return;
        }
        var isfirst = "";
        var islast = "";
        var moveid = "";
        var movetype = "";
        moveid = $("#" + Hidistro_designer.CurrentContentID).next().attr("id");
        movetype = $("#" + Hidistro_designer.CurrentContentID).next().attr("type");
        if (moveid != "" && moveid != undefined) {
            Hidistro_designer.MoveElementId = moveid;
            Hidistro_designer.MoveType = movetype;
            $("#" + moveid).after("<div id='moveDiv'></div>");
            $("#" + Hidistro_designer.CurrentContentID).hide();
            $("#moveDiv").replaceWith($("#" + Hidistro_designer.CurrentContentID));
            $("#" + Hidistro_designer.CurrentContentID).show();
        }
        ...
        $(ctrnamediv).css({ 'top': $("#" + Hidistro_designer.CurrentContentID).offset().top + ($("#" + Hidistro_designer.CurrentContentID).outerHeight() / 2 - 12), 'left': $("#" + Hidistro_designer.CurrentContentID).offset().left + ($("#" + Hidistro_designer.CurrentContentID).outerWidth() / 2) - 80 });
        Hidistro_designer.MoveButtonState();
        Hidistro_designer.EditeMove(2);
    },   //向下移动
    MoveButtonState: function () {
        $(a_up).attr("class", "up");
        $(a_down).attr("class", "down");
        var firstid = $("#" + Hidistro_designer.CurrentContentID).parent().children().first().attr("id");

        var lastid = $("#" + Hidistro_designer.CurrentContentID).parent().children().last().attr("id");

        if (firstid == Hidistro_designer.CurrentContentID) {
            $(a_up).attr("class", "updisable");
        }
        if (lastid == Hidistro_designer.CurrentContentID) {
            $(a_down).attr("class", "downdisable");
        }
    },
    BindSingleEvent: function (singleobj, isevent) {
        var currentsignle = "#" + singleobj; //获取当前传过来的id值
        if (Hidistro_designer.IsParent) {//获取到父级
            //            $(currentsignle).hover(function () {
            //                $(parentmove).css({ top: $(currentsignle).offset().top, left: $(currentsignle).offset().left + $(currentsignle).width() }).show('slow');
            //            }, function () {
            //                if (Hidistro_designer.IsParent) {
            //                    $(parentmove).hide('hide');
            //                }
            //            });
        } else {
            ...
            $(currentsignle).mouseenter(function () {
                var currentdesign = this;
                $(currentdesign).focus();
                if (Hidistro_designer.IsShow && !Hidistro_designer.MouseDown
            && singleobj != Hidistro_designer.MoveElementId) {//当窗口为显示状态并且当前层不为移动层时
                    ...
                    if ($(currentdesign).attr("IsEdit") == "true" || $(currentdesign).attr("del") == "no") { //当层属性只为编辑或者不可删除时
                        $(a_down).hide();
                        $(a_up).hide();
    ...
    EditeMove: function (direction) {
        $.ajax({
            url: desigurl,
            type: "post",
            cache: false,
            data: {
                ModelId: "editedirection",
                Direction: direction,
                Type: Hidistro_designer.CurrentcontrolType,
                MoveType: Hidistro_designer.MoveType,
                Elementid: Hidistro_designer.CurrentContentID,
                MoveElementid: Hidistro_designer.MoveElementId,
                PageName: Hidistro_designer.CurrentPageName,
                rand: Math.random()
            },
            dataType: "json",
            success: function (datas) {
                if (datas.success) {
                    Hidistro_Dialog.dialogTip("succeed", datas.Result);
                } else {
                    Hidistro_Dialog.dialogTip("error", datas.Result);
                }
            },
            error: function (errorText,errorCode) {
                Hidistro_Dialog.dialogTip("error", errorCode);
            }
        });
    },
    ShowDialog: function () {
