...
function getReturnUrl() {
    var listurl = "/Admin/Supplier/Product/ProductList";
    var url = listurl + "?";
    if (queryParameters) {
...
function InitPage() {
    ...
    $("#btnUnSale").on("click", function () {
        ...
        Post_ChangeSaleStatus(ids, 2);
    });
    $("#btnInStock").on("click", function () {
        var ids = GetSelectId();
        if (ids.length == 0) {
            alert("请选择数据项");
            return;
        }
        Post_ChangeSaleStatus(ids, 3);
    });
}
...
function initQuery(obj) {
    ...
    if (BrandId.toString().length > 0) {
        obj.BrandId = BrandId;
    }
    var TagId = $("#dropTagList").val();
    if (TagId.toString().length > 0) {
        obj.TagId = TagId;
    }
    var SaleStatus = $("#hidStatus").val();
    if (SaleStatus.toString().length > 0) {
        obj.SaleStatus = SaleStatus;
    }
    var TypeId = $("#dropType").val();
    if (TypeId.toString().length > 0) {
        obj.TypeId = TypeId;
    }
    if ($("#chkIsWarningStock").is(":checked")) {
        obj.IsWarningStock = true;
    }
    return obj;
}
//参数检测
...
function Post_Delete(ids) {
    ...
    if (canoper) {
        ...
        $.ajax({
            ...
            error: function () {
                try {
                    loading.close(); $('.hishop_menu_scroll', window.parent.document).css("opacity", "1");
                } catch (e) { }
            }
        });
    }
...
function Post_ChangeSaleStatus(ids, status) {
    if (ids.length < 1) {
        ShowMsg("错误的编号", false);
        return;
    }
    ...
    if (canoper) {
        var pdata = {
            ids: ids, status: status, action: "changesalestatus"
        }
        var loading;
        try {
            loading = showCommonLoading();
