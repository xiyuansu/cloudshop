<%@ Page Title="" Language="C#" MasterPageFile="~/Admin/Admin.Master" AutoEventWireup="true" CodeBehind="ShopMenu.aspx.cs" Inherits="Hidistro.UI.Web.Admin.VShop.ShopMenu" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="headHolder" runat="server">
    ...
    <script type="text/javascript" type="text/javascript" src="/utility/jquery.hishopUpload.js" ></script>
    <script type="text/javascript" type="text/javascript" src="/utility/jquery.form.js"></script>
    ...
    <script type="text/javascript">
        ...
        function loadmenu() {
            $.ajax({
                type: "POST",
                url: "../../API/MenuProcess.ashx?action=gettopmenus&clientType=1",
                dataType: "JSON",
                success: function (d) {
                    if (d.status == "0") {
                        showmenu(d);
        ...
        function submitaddandedit() {
            ...
            if (andedit == 0) {//添加一级和二级菜单
                ...
                $.ajax({
                    ...
                    data: { action: "addmenu", ParentMenuId: parentid, Name: escape($("#txttitle").val()), Content: $("#txtContent").val(), Type: Type, ShopMenuPic: pic, clientType: 1 },
                    success: function (d) {
                        if (d.status == "0") {
            ...
            if (andedit == 1)//修改一级和二级菜单
            {
                ...
                $.ajax({
                    url: "../../API/MenuProcess.ashx",
                    type: 'post', dataType: 'json', timeout: 10000,
                    data: { action: "editmenu", ParentMenuId: parentid, MenuId: editid, Name: $("#txttitle").val(), Content: $("#txtContent").val(), Type: Type, ShopMenuPic: pic, clientType: 1 },
                    success: function (d) {
                        if (d.status == "0") {
                            HiTipsShow("修改成功！", 'success');
                            loadmenu();
        ...
        function delmenu(id, type) {
            if (confirm("确定要删除数据吗？")) {
                ...
                $.ajax({
                    type: "POST",
                    url: "../../API/MenuProcess.ashx?action=delmenu" + url,
                    success: function (d) {
                        if (d.status == "0") {
                            if (type == 0) {
                                setmenuid = "";
                        ...
                        <div id="panediv" class=" navgation">
                            <div class=" navgation" id="tabpane" style="border: 1px solid #ddd; padding: 15px;">
                                ...
                                <div class="shop-index clearfix">
                                    ...
                                    <span class="fr"><span style="cursor: pointer;" id="EditMenu">编辑</span>
                                        &nbsp;<span id="spanhid" style="color: #999">|&nbsp;
                                    <small class="inl">设置二级导航以后，主链接已失效。</small>
                                        </span></span>
                                </div>
