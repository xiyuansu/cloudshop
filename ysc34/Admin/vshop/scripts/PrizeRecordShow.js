...
function initQuery(obj) {
    var ActivityId = $("#hidActiveid").val();
    if (ActivityId) {
        if (ActivityId.toString().length > 0) {
            obj.ActivityId = ActivityId;
        }
    }
    var UserName = $("#txtUserNameText").val();
    if (UserName.length > 0) {
        obj.UserName = UserName;
    }
    var AwardGrade = $("#selectAwardItem").val();
    if (AwardGrade.toString().length > 0) {
        obj.AwardGrade = AwardGrade;
    }
    var Status = $("#ddlStateus").val();
