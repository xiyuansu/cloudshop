...
function initQuery(obj) {
    ...
    var PayStart = $("#calendarPayStart").val();
    if (PayStart.length > 0) {
        obj.PayStart = PayStart;
    }
 
    var PayEnd = $("#calendarPayEnd").val();
    if (PayEnd.length > 0) {
        obj.PayEnd = PayEnd;
    }
    var SplittingTypes = $("#ddlSplittinTypes").val();
    if (SplittingTypes.length > 0) {
        obj.SplittingTypes = SplittingTypes;
    }
    var SetStart = $("#calendarSetStart").val();
    if (SetStart.length > 0) {
        obj.SetStart = SetStart;
    }
    var SetEnd = $("#calendarSetEnd").val();
    if (SetEnd.length > 0) {
        obj.SetEnd = SetEnd;
    }
