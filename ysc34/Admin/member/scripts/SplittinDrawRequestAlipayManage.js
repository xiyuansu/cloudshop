...
function ShowCancelReCharge(opers, id, UserName) {

    $('#ChargeType').val('CancelReCharge');
    $('#JournalNumber').val(id);
    $('#lblcancelUserName').html(UserName);

    DialogShow(opers, 'balancedrawrequest', 'divcancelrecharge', 'btnRequest');
}
...
function OpenNewWin(IDList) {
    window.open("BalanceDrawRequestOnLine.aspx?Ids=" + IDList + "&Type=splittin", "_blank");
}
