...
<asp:Content ID="Content2" ContentPlaceHolderID="validateHolder" runat="server">
    <script type="text/javascript" language="javascript">
        ...
        function SelectPromoteType() {
            ...
            if (promoteType == 11)
                $("#radPromoteType_FullAmountDiscount").iCheck('check');
            else if (promoteType == 12)
                $("#radPromoteType_FullAmountReduced").iCheck('check');
            else if (promoteType == 13)
                $("#radPromoteType_FullQuantityDiscount").iCheck('check');
            else if (promoteType == 14)
                $("#radPromoteType_FullQuantityReduced").iCheck('check');
            else if (promoteType == 15)
                $("#radPromoteType_FullAmountSentGift").iCheck('check');
            else if (promoteType == 16)
                $("#radPromoteType_FullAmountSentTimesPoint").iCheck('check');
            else if (promoteType == 17)
                $("#radPromoteType_FullAmountSentFreight").iCheck('check');
        }
        ...
        function Valid() {
            ...
            if (promoteType == 11) {
                ...
                if (!numexp.test(condition)) {
                    ShowMsg("输入满足数量有误(必须是数值)，请重新输入正确的满足数量！");
                    return false;
                }
                if (discountValue.length == 0) {
                    ShowMsg("请输入折扣值(一般在0.01-1之间)！");
                ...
                if (!exp.test(discountValue)) {
                    ShowMsg("折扣值有误(必须是数值)，请重新输入正确的折扣值！");
                    return false;
                }
            }
            else if (promoteType == 15) {
            ...
            if (isOpenStore && restoreStoreId().length == 0) {
                ShowMsg("请选择门店范围！")
                return false;
            }
            return true;
                ...
                <div class="formitem ">
                    <ul>
                        <li><span class="formitemtitle"><em>*</em>促销活动类型：</span>
                            <Hi:PromoteTypeRadioButtonList IsProductPromote="false" runat="server" ID="radPromoteType" IsCanEdit="false"/>
                      ...
                      <%-- <li id="liPromoteTypeDiscount" style="display: none;">
                        ...
                        <li style="margin-top:10px;"><span class="formitemtitle">&nbsp;</span>
                            
                             <asp:Button ID="btnSave" runat="server" Text="保存" OnClientClick="setChooseGifts();return Valid();" CssClass="btn btn-primary" />
                        </li>
