...
function getProductId(){
    ...
    if (productId <= 0) {
        productId = parseInt($(artDialog.open.origin.document.getElementById("ctl00_contentHolder_hidProductId")).val());
    }
    return productId;
}
...
function ShowListData(page, initpagination) {
    ...
    var urldata = {
        pageIndex: page, pageSize: curpagesize, action: "getstores", key: $("#txtStoresName").val(), productId: productId, regionId: $("#regionSelectorValue").val(), tagId: $("#ctl00_contentHolder_ddlTags").val(),activityId: GetQueryString("CountDownId")
    }
    $.ajax({
        type: "GET",
        url: "/admin/promotion/ashx/CountDowns.ashx",
        data: urldata,
        dataType: "json",
        success: function (data) {
			...
			$('.cdstoreH').closest('.aui_state_focus').css('top','0px');
        },
        error: function () {
...
function getAllStoreEnable() {
    ...
    {
        var urldata = {
            action: "getenablestores", key: $("#txtStoresName").val(), productId: getProductId(), regionId: $("#regionSelectorValue").val(), tagId: $("#ctl00_contentHolder_ddlTags").val(), activityId: GetQueryString("CountDownId")
        }
        $.ajax({
            type: "GET",
            url: "/admin/promotion/ashx/CountDowns.ashx",
...
//添加
function SelectCountDownStores() {
    ...
    origin.storeIdArry = storeIdArry;//每次添加的时候给父页面的storeIdArry数组赋值。用作数据校验。
    art.dialog.close();
}