...
function initQuery(obj) {
    var StartDate = $("#CPStartTime").val();
    if (StartDate.length > 0) {
        obj.StartDate = StartDate;
    }
    var EndDate = $("#CPEndTime").val();
    if (EndDate.length > 0) {
        obj.EndDate = EndDate;
    }
    var GroupStatus = $("#ddlStatus").val();
    if (GroupStatus.toString().length > 0) {
        obj.GroupStatus = GroupStatus;
    }
    var FightGroupActivityId = $("#hidFightGroupActivityId").val();
    if (FightGroupActivityId.toString().length > 0) {
        obj.FightGroupActivityId = FightGroupActivityId;
    }
    return obj;
}
//参数检测
function checkQuery(obj) {
    var result = true;
    if (obj.FightGroupActivityId) {
        if (obj.FightGroupActivityId < 1) {
