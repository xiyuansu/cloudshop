...
function initQuery(obj) {
    var CouponName = $("#txtCouponName").val();
    if (CouponName.length > 0) {
        obj.CouponName = CouponName;
    }
    var State = $("#ddlStatus").val();
    if (State.toString().length > 0) {
        obj.State = State;
    }
    var ObtainWay = $("#ddlGetType").val();
    if (ObtainWay.toString().length > 0) {
        obj.ObtainWay = ObtainWay;
    }
    return obj;
}
...
function BeforeDelete(id) {
    var CouponId = id;
    var Msg = "确定要执行失效操作吗？失效后已领取的优惠券将不可以使用？";
    $.ajax({
        url: "../../API/ActivitysHandler.ashx",
        type: 'POST', dataType: 'json', timeout: 10000,
        data: {
            action: "BeforeDelCoupon",
            CouponId: CouponId
        },
        async: false,
