<%@ Page Title="" Language="C#" MasterPageFile="~/Admin/Admin.Master" AutoEventWireup="true" CodeBehind="EditCombinationBuy.aspx.cs" Inherits="Hidistro.UI.Web.Admin.promotion.EditCombinationBuy" %>
...
<script type="text/javascript" language="javascript">
    ...
    function ShowAddMainProductDiv() {
        ...
        DialogFrameClose("product/SearchCombinationBuyProduct.aspx?IsSingle=1&ProductIds=" + cannotChoose(), "选择主商品", null, null, function (e) { CloseFrameWindow(); });
    }
    //组合商品选择
    function ShowAddOtherProductDiv() {
        chooseButton = "otherButton";
        DialogFrameClose("product/SearchCombinationBuyProduct.aspx?IsSingle=0&ProductIds=" + cannotChoose(), "选择组合商品", null, null, function (e) { CloseFrameWindow(); });
    }
    //选择后处理数据
    ...
    function getProductSku(arr) {
        ...
        getSkuDataByProductId(productIds);
    }
    //根据商品IDS获取规格数据
    function getSkuDataByProductId(productIds) {
        var url = '../../API/CombinationBuyHandler.ashx?action=GetSkuByProductIds&ProductIds=' + productIds;
    ...
    function bindMainProductHtml(items) {
        ...
        if (items.length == 1 && n.SkuContent == "") {
            ...
            $("#spanProductName").html(n.ProductName);
            isTableData = false;
        }
        else {
            //$("#divSingleSku").hide();
            ...
            for (var i = 0; i < items.length; i++) {
                var k = items[i];
                if (i == 0) {
                    $("#spanProductName").html(k.ProductName);
                }
                var cprice = parseFloat(k.CombinationPrice);
                content += String.format("<tr name='appendlist'><td width='30%' style='text-align: left;'>{0}</td><td width='30%' style='text-align: left;'>{1}</td><td width='40%' style='text-align: left;'><input type='text' id='hidSku_{2}' value='{3}' onblur='checkIsMoney(this)'  class='forminput form-control' style='width:100px' /></td></tr>", k.SkuContent, k.SalePrice, k.SkuId, cprice);
            }
    ...
    function onSaveCombinationBuy() {
        ...
        var editType = $("#ctl00_contentHolder_hidEditType").val();
        if (editType != "2") {
    ...
    $(document).ready(function () {
        InitValidators();
        var mainProductId = $("#ctl00_contentHolder_hidMainProductId").val();
        var otherProductIds = $("#ctl00_contentHolder_hidOtherProductIds").val();
        var combinationId = $("#ctl00_contentHolder_hidCombinationId").val();
        var editType = $("#ctl00_contentHolder_hidEditType").val();
        if (editType == "2") {
            $("#btnChooseMainProduct").hide();
            $("#ctl00_contentHolder_calendarStartDate").attr("disabled", true);
            $("#ctl00_contentHolder_calendarStartDate").css("background-color", "#ccc");
        }
        $.post('../../API/CombinationBuyHandler.ashx?action=GetSkuByProductIds&ProductIds=' + mainProductId + '&fromTable=combination&CombinationId=' + combinationId, function (result) {
            var items = eval(result);
            if (items.length > 0) {
                bindMainProductHtml(items);
                $.post('../../API/CombinationBuyHandler.ashx?action=GetSkuByProductIds&ProductIds=' + otherProductIds + '&fromTable=combination&CombinationId=' + combinationId, function (re) {
                    var detailitems = eval(re);
                    if (detailitems.length > 0) {
                        bindOtherProductHtml(detailitems);
                    }
                });
            }
        });
...
<asp:Content ID="Content3" ContentPlaceHolderID="contentHolder" runat="server">
    ...
    <asp:HiddenField runat="server" ID="hidCombinationId" />
    <asp:HiddenField runat="server" ID="hidEditType" />
            ...
            <div class="formitem validator2">
                <ul>
                    <li>
                        ...
                        <input type="button" id="btnChooseMainProduct" value="选择主商品" class="btn btn-default float" onclick="ShowAddMainProductDiv()" />
                    </li>
                    <li id="divSingleSku1">
                    ...
                    <asp:Button ID="btnAddCoupons" runat="server" Text="修改" OnClientClick="return onSaveCombinationBuy();" CssClass="btn btn-primary ml_198" />
                </ul>
            </div>
        </div>
    </div>
