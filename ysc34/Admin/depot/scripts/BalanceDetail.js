...
function initQuery(obj) {
    ...
    if (TradeType.toString().length > 0) {
        ...
        success: function (data) {
            ...
            if (data) {
                ...
                if (data.Result.rows.length > 0) {
                    isnodata = false;
                    databox.html(template('datatmpl', data.Result));
                    DataRedraw(databox);
                }
            }
            if (isnodata) {
...
function ReloadPageData(pageindex, initpage) {    /*复位*/    try { var chkall = $("#checkall"); if (chkall) { if (chkall.iCheck) { chkall.iCheck('uncheck'); } chkall.prop("checked", false); } } catch (e) { }
    var _page = showpager;
    ...
    //    initpage = true;
    //}
    ShowListData(showbox, curpageindex, curpagesize, initpage);
    if (hasPaginator && !initpage) {
        _page.HiPaginator("redraw");
    }
}
