...
function initQuery(obj) {
    ...
    var EquipmentId = $("#txtSearchEquipmentId").val();
    if (EquipmentId.toString().length > 0) {
        obj.EquipmentId = EquipmentId;
    }
    var Status = $("#ddlDeviceStatus").val();
...
$("#btnEditRemark").click(function () {
    ...
    var id = $("#hfDeviceId").val();
    var remark = $("#txtWXRemark").val();

    Post_Eidt(id, remark);
});
function Post_Eidt(ids, remark) {
    ...
    var pdata = {
        ids: ids, action: "eidt",Remark:remark
    }
        var loading;
        try {
...
var formtype;
function editRemark(deviceId, remark) {
    formtype = "editRemark";
    $("#hfDeviceId").val(deviceId);
    setArryText("txtWXRemark", remark);
    DialogShow("修改备注", "frameexportcoup", "dvEditRemark", "btnEditRemark");
