...
(function ($) {
    ...
    $.fn.hishopUpload.methods = {
        ...
        getImgHref: function (jQ) {
            var images = $(jQ).find('input[name="iptImgUrl"]');
        if (images.length == 1)
            return images.val();
        else {
            var srcArr = [];
    ...
    var renderView = {
        render: function (opts, container) {
            ...
            for (var i = 0; i < opts.imagesCount; i++) {
                ...
                var imghrefItem = opts.displayImgHref[i] ? opts.displayImgHref[i] : '';
                html += ' <div class="upload-img-box" imageBox ' + display + '>\
                                 ...
                                 <input class="form_input form-control" type="text" placeholder="链接地址,外链请加上 http:// 前缀" name="iptImgUrl"  value="' + imghrefItem + '"/>\
