...
function ltrim(str) { //删除左边的空格
    return (str == null || str == undefined) ? str : str.replace(/(^\s*)/g, "");
}
...
function initValid(validator) {
    ...
    if (srcTag == "INPUT" || srcTag == "TEXTAREA") {
        ...
        if (type == "text" || type == "password" || type == "file" || srcTag == "TEXTAREA") {
            var defaultVal = trim(controlToValidate.attr("value"));//初始加上了 去掉字符串左右空格
            ...
            controlToValidate.change(function () {
                for (var i = 0; i < this.validator.length; i++) {
                    this.validator[i].valid();
                    if (this.validator[i].isAjax == null || this.validator[i].isAjax == undefined) {
                        this.validator[i].UpdateStatus();
...
function removeVerification(controlIds) {
    var controls = controlIds.split(",");
    for (var i = 0; i < controls.length; i++) {
        if (controls[i] == "")
            continue;
        $("#" + controls[i]).removeAttr("validategroup").removeClass("errorFocus");
