$(function () {
    ...
    HiShop.DIY.Unit.event_type1 = function (ctrldom, data) {
        ...
        data.ue = UE.getEditor('editor' + data.id, { autoHeightEnabled: false}); //创建编辑器       
        ...
        $ctrl.find("#close_edit").click(function () {
            $(this).closest(".diy-ctrl-item").hide();
            $(".modal").remove();
        })
    ...
    HiShop.DIY.Unit.event_type5 = function (ctrldom, data) {
        ...
        var reRender = function () {
            var categoryId = $("#dropCategories1").val();
            var $render = $(_.template(tpl_con, data));
            $conitem.find(".members_con").remove().end().append($render);

            var $render_ctrl = $(_.template(tpl_ctrl, data));
            $ctrl.empty().append($render_ctrl);
            $("#dropCategories1").val(categoryId);
        ...
        //分类选择
        $ctrl.find("select[id='dropCategories1']").change(function () {
            var val = $(this).val();
            data.content.categoryId = val;//同步数据到缓存
            reRender();
        });
        //是否第一级优先
    ...
    HiShop.DIY.Unit.event_type12 = function (ctrldom, data) {
        ...
        $ctrl.find('.ctrl-item-list-li').each(function (index, ell) {
            ...
            me.find('.colorPicker').each(function (indexs, el) {
                ...
                var bgcolor = data.content.dataset[index].bgColor;
                console.log(bgcolor);
                $(".diy-phone-contain").css("background", bgcolor);           
        });
        // 上传导航小图标
