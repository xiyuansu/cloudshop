<%@ Page Title="" Language="C#" MasterPageFile="~/Admin/Admin.Master" AutoEventWireup="true" CodeBehind="ShopMenu.aspx.cs" Inherits="Hidistro.UI.Web.Admin.store.ShopMenu" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="headHolder" runat="server">
    ...
    <script type="text/javascript">
        ...
        function loadmenu() {
            $.ajax({
                type: "POST",
                url: "../../API/MenuProcess.ashx?action=gettopmenus&clientType=0",
                dataType: "JSON",
                success: function (d) {
                    if (d.status == "0") {
                        showmenu(d);
        ...
        function submitaddandedit() {
            ...
            if (andedit == 0) {//添加一级和二级菜单
                ...
                $.ajax({
                    ...
                    data: { action: "addmenu", ParentMenuId: parentid, Name: escape($("#txttitle").val()), Content: $("#txtContent").val(), Type: Type, ShopMenuPic: pic, clientType: 0 },
                    success: function (d) {
                        if (d.status == "0") {
            ...
            if (andedit == 1)//修改一级和二级菜单
            {
                ...
                $.ajax({
                    url: "../../API/MenuProcess.ashx",
                    type: 'post', dataType: 'json', timeout: 10000,
                    data: { action: "editmenu", ParentMenuId: parentid, MenuId: editid, Name: $("#txttitle").val(), Content: $("#txtContent").val(), Type: Type, ShopMenuPic: pic, clientType: 0 },
                    success: function (d) {
                        if (d.status == "0") {
                            HiTipsShow("修改成功！", 'success');
                            loadmenu();
