...
function initQuery(obj) {
    ...
    var FromDate = $("#calendarStartDataTime").val();
    if (FromDate.toString().length > 0) {
        obj.FromDate = FromDate;
    }
    var ToDate = $("#calendarEndDataTime").val();
...
function Release(ArticleId, IsRelease) {

    var pdata = {
        IsRelease: IsRelease, ArticleId: ArticleId, action: "release"
    }
    var loading;
    try {
        loading = showCommonLoading();
