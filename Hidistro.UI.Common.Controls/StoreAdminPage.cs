...
{
	public class StoreAdminPage : Page
	{
		...
		protected virtual void RegisterFrameScript()
		{
			...
			string script = string.Format("<script>if(window.parent.frames.length == 0) window.location.href=\"{0}\";</script>", "/depot/default.html");
		...
		private void CheckPageAccess()
		{
			...
			else if (!Users.GetStoreState(HiContext.Current.Manager.StoreId))
			{
				this.Page.Response.Redirect("/Depot/LoginExit.aspx", true);
			}
		}
	}
}
