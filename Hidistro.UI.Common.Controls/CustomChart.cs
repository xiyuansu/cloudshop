...
namespace Hidistro.UI.Common.Controls
{
	public class CustomChart
	{
		...
		{
			private Color[] _color = new Color[12]
			{
				Color.Chocolate,
				Color.YellowGreen,
				Color.Olive,
		...
		public class PieChart : Chart
		{
			...
			public void CollectDataPoints(string[] xValues, string[] yValues)
			{
				...
				for (int i = 0; i < xValues.Length; i++)
				{
					...
					this._chartItems.Add(new ChartItem(xValues[i], xValues.ToString(), num, 0f, 0f, Color.AliceBlue));
					this._total += num;
				}
				float startPos = 0f;
				int num2 = 0;
				foreach (ChartItem chartItem in this._chartItems)
				{
		...
		public class BarGraph : Chart
		{
			...
			public void CollectDataPoints(string[] labels, string[] values)
			{
				...
				for (int i = 0; i < labels.Length; i++)
				{
					...
					base.DataPoints.Add(new ChartItem(text, labels[i], num, 0f, 0f, base.GetColor(i)));
