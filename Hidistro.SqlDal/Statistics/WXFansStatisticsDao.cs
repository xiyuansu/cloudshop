...
{
	public class WXFansStatisticsDao : BaseDao
	{
		public PageModel<WXFansStatisticsInfo> GetWxFansList(WXFansStatisticsQuery query)
		{
			...
			return DataHelper.PagingByRownumber<WXFansStatisticsInfo>(query.PageIndex, query.PageSize, sortBy, sortOrder, true, "Hishop_WXFansStatistics", "Id", stringBuilder.ToString(), "*");
		}

		public IList<WXFansStatisticsInfo> GetWxFansListNoPage(WXFansStatisticsQuery query)
		{
			IList<WXFansStatisticsInfo> result = new List<WXFansStatisticsInfo>();
			string format = "SELECT * FROM Hishop_WXFansStatistics WHERE {0} ORDER BY {1}";
			StringBuilder stringBuilder = new StringBuilder(" 1 = 1 ");
			DateTime dateTime;
			switch (query.LastConsumeTime)
			{
			...
			{
				result = DataHelper.ReaderToList<WXFansStatisticsInfo>(objReader);
			}
			return result;
		}
		...
		{
			DbCommand sqlStringCommand = base.database.GetSqlStringCommand("DELETE FROM Hishop_WXFansStatistics");
			base.database.ExecuteNonQuery(sqlStringCommand);
		}

		public bool IsExistData(DateTime dt)
		{
			DbCommand sqlStringCommand = base.database.GetSqlStringCommand(string.Format("SELECT COUNT(*) FROM Hishop_WXFansStatistics WHERE  DATEDIFF(dd,StatisticalDate,'{0}') = 0 ", dt.ToString("yyyy-MM-dd")));
			return base.database.ExecuteScalar(sqlStringCommand).ToInt(0) > 0;
		}

		public DateTime? GetLastStatisticalDate()
		{
			DbCommand sqlStringCommand = base.database.GetSqlStringCommand("SELECT TOP 1 StatisticalDate FROM Hishop_WXFansStatistics ORDER BY StatisticalDate DESC");
