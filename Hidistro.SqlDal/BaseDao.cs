...
namespace Hidistro.SqlDal
{
	public class BaseDao
	{
		...
		public virtual bool SaveSequence<T>(int keyId, int sequence, string keyName = null) where T : new()
		{
			...
			{
				Globals.WriteExceptionLog(ex, null, "SaveSequence");
			}
			return result;
		}
		...
		public string GetTrueOrderId(string orderId)
		{
			if (!string.IsNullOrEmpty(orderId) && orderId.Length >= 18)
			{
				if (orderId.Length >= 20)
				{
					return orderId.Substring(0, orderId.Length - 5);
				}
