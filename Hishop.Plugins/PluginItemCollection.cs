	...
	public class PluginItemCollection
	{
		...
		public virtual string ToJsonString()
		{
			...
			if (this.plugins.Count > 0)
			{
				...
				foreach (string key in this.plugins.Keys)
				{
					PluginItem pluginItem = this.plugins[key];
					stringBuilder.Append("{");
					stringBuilder.AppendFormat("\"FullName\":\"{0}\",\"DisplayName\":\"{1}\",\"Logo\":\"{2}\",\"ShortDescription\":\"{3}\",\"Description\":\"{4}\"", pluginItem.FullName, pluginItem.DisplayName, pluginItem.Logo, pluginItem.ShortDescription, pluginItem.Description);
					stringBuilder.Append("},");
				}
		...
		public virtual string ToXmlString()
		{
			...
			foreach (string key in this.plugins.Keys)
			{
				...
				stringBuilder.AppendFormat("<FullName>{0}</FullName><DisplayName>{1}</DisplayName><Logo>{2}</Logo><ShortDescription>{3}</ShortDescription><Description>{4}</Description>", pluginItem.FullName, pluginItem.DisplayName, pluginItem.Logo, pluginItem.ShortDescription, pluginItem.Description);
				stringBuilder.Append("</item>");
			}
			stringBuilder.Append("</xml>");
			return stringBuilder.ToString();
